package synthetic.placeholderutil.color;

import me.clip.placeholderapi.expansion.PlaceholderExpansion;
import net.md_5.bungee.api.ChatColor;
import org.bukkit.entity.Player;
import org.jetbrains.annotations.NotNull;
import synthetic.placeholderutil.utils.CounterUtil;
import synthetic.placeholderutil.utils.RGBUtil;

import java.awt.*;

public class ColorCustomWaveExpansion extends PlaceholderExpansion {
    @Override
    public @NotNull String getIdentifier() {
        return "phucw";
    }

    @Override
    public @NotNull String getAuthor() {
        return "onXans#1772";
    }

    @Override
    public @NotNull String getVersion() {
        return "1.0.0";
    }

    @Override
    public String onPlaceholderRequest(Player player, String identifier) {

        String[] parts = identifier.replace('-',' ').split("_");
        if(parts.length == 4) {
            String color1 = parts[0];
            String color2 = parts[1];
            String format = parts[2];
            String text = parts[3];

            if(text.equals("#name#")) text = player.getDisplayName();

            StringBuilder outputBuilder = new StringBuilder();

            Color Jcolor1 = Color.decode(color1);
            Color Jcolor2 = Color.decode(color2);


            int offset = 0;
            for(Character c : text.toCharArray()) {

                outputBuilder.append(ChatColor.of(RGBUtil.interColorWave(CounterUtil.rapid,Jcolor1,Jcolor2,offset,5)));

                if(format.contains("l")) outputBuilder.append("§l");
                if(format.contains("n")) outputBuilder.append("§n");
                if(format.contains("m")) outputBuilder.append("§m");
                if(format.contains("o")) outputBuilder.append("§o");

                outputBuilder.append(c);
                offset++;
            }
            outputBuilder.append("§r");
            return outputBuilder.toString();
        }
        return "";
    }
}
